...
#define LOG_ERR(fmt, ...)                                                      \
  fprintf(stderr, COLOR_RED "[×] " fmt COLOR_RESET "\n", ##__VA_ARGS__)
...
static const unsigned char mach_thread_code[] = {
    ...
    0xff, 0xd0,       // 000000011F9B8008 call    rax ;
                      // _pthread_create_from_mach_thread
    0x48, 0xc7, 0xc0, 0x09, 0x03, 0x00,
    0x00,                        // 000000011F9B800A mov     rax, 309h ; 777
...
static const unsigned char posix_thread_code[] = {
    ...
    0x48, 0x89,
    0xe5, // 00000001210EE002                 mov     rbp, rsp
    0x48, 0x8b,
    0x07, // 00000001210EE005                 mov     rax, [rdi]      ; dlopen
    0x48, 0x8b, 0x7f,
    0xf8, // 00000001210EE008                 mov     rdi, [rdi-8]    ;
          // xxx.dylib
    0xbe, 0x01, 0x00, 0x00,
    0x00, // 00000001210EE00C                 mov     esi, 1
    0xff,
    0xd0, // 00000001210EE011                 call    rax             ; dlopen
...
unsigned char mach_thread_code[] = {
    //    "\x20\x8e\x38\xd4" //   brk
    "\x80\x00\x3f\xd6" // 0x121858004: blr    x4
...
extern "C" int pthread_create_from_mach_thread(pthread_t *thread,
...
bool is_dylib_loaded2(const task_t task, const char *dylib_path) {
  ...
  mach_vm_read(task, (mach_vm_address_t)infos->infoArray, size, &readData,
               &dataCnt);
  ...
  for (int i = 0; i < (infos->infoArrayCount); i++) {
    size = PATH_MAX;
    mach_vm_read(task, (mach_vm_address_t)info[i].imageFilePath, size,
                 &readData, &dataCnt);
...
int inject_dylib_x86(pid_t pid, const char *lib) {
  ...
  kr(mach_vm_allocate(task, &posix_code_mem, MACH_CODE_SIZE,
                      VM_FLAGS_ANYWHERE));
  ...
  kr(mach_vm_allocate(task, &posix_param_mem, (PTR_SIZE * 2),
                      VM_FLAGS_ANYWHERE));

  // Write the path into memory.
  kr(mach_vm_write(task, path_mem, (vm_offset_t)lib, (int)path_length));
  // Write the parameter array contents into memory. This array will be the
  // pthread's parameter. The address of dlopen() is the first parameter.
  kr(mach_vm_write(task, posix_param_mem, (vm_offset_t)&dlopen_address,
                   PTR_SIZE));
  // The pointer to the dylib path is the second parameter.
  kr(mach_vm_write(task, (posix_param_mem - PTR_SIZE), (vm_offset_t)&path_mem,
                   PTR_SIZE));
  // Write to both instructions, and mark them as readable, writable, and
  // executable. Do it for the mach thread instruction.
  kr(mach_vm_write(task, mach_code_mem, (vm_offset_t)&mach_thread_code,
                   MACH_CODE_SIZE));
  // Do it for the pthread instruction.
  kr(mach_vm_write(task, posix_code_mem, (vm_offset_t)&posix_thread_code,
                   POSIX_CODE_SIZE));

  kr(mach_vm_protect(task, mach_code_mem, MACH_CODE_SIZE, FALSE, VM_PROT_ALL));
  kr(mach_vm_protect(task, posix_code_mem, POSIX_CODE_SIZE, FALSE,
                     VM_PROT_ALL));

  // The state and state count for launching the thread and reading its
  // registers.
  ...
  thread_act_t thread;
  kr(thread_create_running(task, state, (thread_state_t)(&regs), state_count,
                           &thread));
...
int inject_dylib_arm(pid_t pid, const char *lib) {
  ...
  kr(mach_vm_allocate(task, &remote_mach_code, MACH_CODE_SIZE,
                      VM_FLAGS_ANYWHERE));
  ...
  kr(mach_vm_write(task, remote_mach_code, (vm_address_t)mach_thread_code,
                   MACH_CODE_SIZE));
  kr(mach_vm_protect(task, remote_mach_code, MACH_CODE_SIZE, FALSE,
  ...
  kr(thread_create_running(task, ARM_THREAD_STATE64, (thread_state_t)&regs,
                           ARM_THREAD_STATE64_COUNT, &remote_thread));
...
pid_t find_pid_by_name2(const char *process_name) {
  ...
  for (int i = 0; i < proc_count; i++) {
    ...
    const int st = proc_pidinfo(pids[i], PROC_PIDTBSDINFO, 0, &proc,
    ...
    if (st == PROC_PIDTBSDINFO_SIZE) {
      ...
      if (strcmp(process_name, proc.pbi_name) == 0) {
        LOG_OK("Found target process: %s (PID: %d)", proc.pbi_name,
               proc.pbi_pid);
        retpid = pids[i];
        break;
      }
...
pid_t find_pid_by_name(const char *process_name) {
  ...
  LOG_INFO("Scanning %zu processes for target '%s'", process_count,
           process_name);
...
kern_return_t wait_for_dyld(pid_t pid) {
  ...
  while (retries < MAX_RETRIES) {
    ...
    if (dyld_info.all_image_info_addr != 0) {
      LOG_OK("dyld images are loaded. Address: %p",
             (void *)dyld_info.all_image_info_addr);
      return KERN_SUCCESS;
    }

    retries++;
    LOG_WARN("dyld images are not loaded yet. Retrying... (%d/%d)", retries,
             MAX_RETRIES);
    usleep(1000);
  }
  ...
  @autoreleasepool {
    if (argc < 2) {
      fprintf(
          ...
          "  %s /Applications/Calculator.app/Contents/MacOS/Calculator "
          "/path/to/library.dylib -spawn\n\n"
    ...
    if (argc > 3) {
      if (strcmp(argv[3], "-name") == 0) {
        while (1) {
          ...
          LOG_WARN("Process not found. Retrying...");
          usleep(100000);
        }
      }
      if (strcmp(argv[3], "-spawn") == 0) {
        ...
        // "/dev/null", O_WRONLY, 0);
        int spawn_result =
            posix_spawn(&pid, process, &file_actions, &attr,
                        (char *const[]){(char *)process, NULL}, environ);
        // int spawn_result = posix_spawn(&pid, process, NULL, NULL, (char
        // *const[]){(char *)process, NULL}, environ);
